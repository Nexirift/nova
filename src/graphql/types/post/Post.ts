...
import { db } from "@/db";
import type { PostSchemaType } from "@/db/schema";
  ...
  fields: (t) => ({
    ...
    replies: t.field({
      ...
      resolve: async (parent, args) => {
        const result = await db.query.post.findMany({
          where: (post, { and, eq }) =>
            and(eq(post.parentId, parent.id), eq(post.quoted, false)),
          limit: args.first!,
          offset: args.offset!,
        });
        return result!;
    ...
    content: t.exposeString("content", {
      authScopes: async (parent, _args, context) => {
        const user = await getUser(parent.authorId, context.auth?.user.id);
    ...
    giveaway: t.field({
      ...
      resolve: async (post) => {
        const result = await db.query.postGiveaway.findFirst({
          where: (postGiveaway, { eq }) => eq(postGiveaway.postId, post.id),
        });
        return result!;
      },
    }),
    ...
    repostsCount: t.field({
      ...
      resolve: async (parent) => {
        ...
        return (repostsResult?.length ?? 0) + (quotesResult?.length ?? 0);
      },
    }),
