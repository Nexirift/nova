...
import { PostInteraction } from "@/graphql/types";
import {
  postInteraction,
...
async function postInteract(
  ...
  if ((await privacyGuardian(post.author, ctx.auth)) === false) {
    return throwError("You cannot interact with this post.", "POST_PRIVACY");
  }
  ...
  if (["LIKE", "REPOST"].includes(type)) {
    ...
    if (!existingInteraction) {
      return throwError(
        errorMap[type],
        `POST_NOT_${type.startsWith("UN") ? type.replace("UN", "") : type}ED`,
      );
    }
    ...
    const result = await db
      ...
      .where(
        and(
          eq(postInteraction.userId, ctx.auth?.user?.id),
          eq(postInteraction.postId, args.id),
          eq(postInteraction.type, type.replace("UN", "") as "LIKE" | "REPOST"),
        ),
      )
      .returning();
    return result[0] || null;
